...
- 视图页可实时查看所选账户的种子任务，展示动态进度、做种/下载用户、双向速率、标签、添加时间、Tracker 与保存路径，并每 2 秒自动同步。
- 视图页支持名称、大小等字段的正倒序排序、关键词搜索，以及状态、路径、标签、Tracker 的多选筛选。
- 任务表头支持右键显示/隐藏可选列、左右移动列；名称和大小固定在前两列。各列可拖拽调节宽度，拖拽不会误触排序，布局与排序偏好保存在当前浏览器。
- 支持新增、双击编辑、删除和拖拽排序横栏。
- 支持卡片绑定名称、保存路径、标签和封面。
- 卡片右键进入设置，可修改卡片、删除卡片、维护标签池。
- 标签池可复用，标签和莫奈封面使用低饱和配色。
- 卡片支持上传单个或多个 `.torrent` 文件，并按预设保存路径和标签添加到 qBittorrent。
- 域流页按 Tracker 域名统计近 1 天、近 1 周或近月内的上传与下载流量，饼图与域名图例分栏展示，时间范围在当前浏览器标签页中记忆。
- 视图、域流与操作日志页面的手动刷新按钮提供单圈旋转反馈，自动同步不会干扰按钮状态。
