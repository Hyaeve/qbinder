    ...
    <div v-if="scheduleEditor.open" class="modal-backdrop" @click.self="closeScheduleEditor">
      <form class="modal schedule-editor" @submit.prevent="saveSchedule">
        ...
        <template v-if="requiresScheduleTargets">
          <div class="schedule-filter"><div class="schedule-filter-heading"><strong>选择种子</strong><span>已选 {{ scheduleEditor.hashes.length }} 个</span></div><div class="schedule-filter-columns"><section class="schedule-filter-status"><small>状态</small><label v-for="option in statusOptions" :key="option.key"><input v-model="scheduleFilter.status" type="checkbox" :value="option.key" /><span>{{ option.label }}</span></label></section><section class="schedule-filter-tags"><small>标签</small><label v-for="tag in scheduleTagOptions" :key="tag" :title="tag"><input v-model="scheduleFilter.tags" type="checkbox" :value="tag" /><span>{{ tag }}</span></label><i v-if="!scheduleTagOptions.length">暂无标签</i></section><section class="schedule-filter-torrents"><small>种子</small><label v-for="task in scheduleFilteredTasks" :key="task.hash" class="schedule-torrent-option"><input v-model="scheduleEditor.hashes" type="checkbox" :value="task.hash" /><span :title="task.name">{{ shortScheduleTaskName(task.name) }}</span></label><i v-if="!scheduleFilteredTasks.length">没有匹配的种子</i></section></div></div>
